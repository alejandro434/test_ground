...
RETRIEVAL_QUERY = """
// --- Retrieval of project metadata from relevant chunk ------------------------
MATCH (node)<-[:HAS_CHUNK]-(p:Project)
OPTIONAL MATCH (p)-[:IN_REGION]->(r:Region)
OPTIONAL MATCH (p)-[:IN_COMMUNE]->(co:Commune)
OPTIONAL MATCH (p)-[:HAS_TIPOLOGIA]->(ti:Tipologia)
OPTIONAL MATCH (p)-[:HAS_PROJECT_TYPE]->(pt:ProjectType)
OPTIONAL MATCH (p)-[:PRESENTED_ON]->(d:PresentationDate)
WITH
  p,
  r.name                           AS region,
  collect(DISTINCT co.name)        AS communes,
  ti.code                          AS tipologia_sea,
  pt.name                          AS tipo_de_proyecto,
  d.date                           AS fecha_de_presentacion
RETURN
  p.name                AS nombre_del_proyecto,
  p.id                  AS id_del_proyecto,
  region,
  communes              AS comunas_involucradas,
  tipologia_sea,
  tipo_de_proyecto,
